...
describe("requestPortOneCheckout", () => {
  ...
  it("overrides the webhook URL when a notice URL is configured", async () => {
    vi.stubEnv("VITE_PORTONE_NOTICE_URL", "https://example.ngrok-free.dev/api/v1/webhooks/portone");
    ...
    expect(requestPayment).toHaveBeenCalledWith(
      expect.objectContaining({
        noticeUrls: ["https://example.ngrok-free.dev/api/v1/webhooks/portone"],
      }),
    );
  });
