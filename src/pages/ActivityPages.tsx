...
export function CouponsPage() {
  return <section className="page-container narrow-page"><PageHeader title="쿠폰 지갑" description="보유한 쿠폰과 사용 내역을 확인하세요." action={<b className="green-text">사용 가능 쿠폰 2장</b>}><Breadcrumbs items={[{ label: '홈', to: '/' }, { label: '쿠폰 지갑' }]} /></PageHeader>
    <h2 className="content-title">보유 쿠폰</h2><div className="coupon-list"><Coupon amount="3,000" content="10,000원 이상 결제 시 · 김해 가야문화 체험 재방문 시" expiry="2026년 8월 31일까지" /><Coupon amount="5,000" content="20,000원 이상 결제 시 · 동해 바다 산책 예약 시" expiry="2026년 9월 15일까지" /></div><p className="coupon-note">쿠폰 적용 가능 여부와 최종 할인 금액은 예약 결제 단계에서 현재 회차와 결제 금액을 기준으로 다시 확인됩니다.</p>
    <h2 className="content-title history-title">사용 · 복구 이력</h2><div className="history-list"><HistoryItem symbol="↶" title="3,000원 할인 · 쿠폰 복구" sub="예약 ID 123 · 카드 실패 ID 9001 · 할인 3,000원" date="복구 2026. 08. 08 10:30" /><HistoryItem symbol="✓" title="지역 체험 5,000원 할인 · 사용 확정" sub="예약 ID 94 · 카드 승인 ID 884 · 할인 5,000원" date="사용 2026. 07. 21 14:05" /></div>
  </section>
}

function Coupon({ amount, content, expiry }: { amount: string; content: string; expiry: string }) { return <article className="coupon"><div><StatusPill>사용 가능</StatusPill><b>{amount}<small>원 할인</small></b></div><div><p>{content}</p><small>방문 인증으로 발급</small></div><time>{expiry}<small>발급일 2026. 08. 01</small></time></article> }
...
export function MissionsPage() {
  ...
  return <section className="page-container narrow-page"><PageHeader title="내 지역 미션" description="참여한 미션의 진행도와 완료 보상을 확인하세요." action={<button className="region-button" onClick={openRegionDialog}>✦ {region} · 지역 변경</button>}><Breadcrumbs items={[{ label: '홈', to: '/' }, { label: '내 지역 미션' }]} /></PageHeader>
    <div className="tab-row"><button className="active">진행 중 <b>2</b></button><button>완료 <b>1</b></button><button>종료됨 <b>1</b></button></div><div className="mission-list"><Mission count="2 / 3" title="김해 문화 한 바퀴" details="김해 가야문화 체험 · 대성동고분박물관 해설 · 낙동강 생태 탐방" date="8. 31. 종료" /><Mission count="1 / 3" title="김해에서 세 번 만나기" details="김해시의 행사·체험을 서로 다른 유형 방문으로 채워 보세요." date="9. 15. 종료" /></div><Notice>안내&nbsp; 완료 보상은 미션이 종료되기 전까지만 수령할 수 있습니다.</Notice>
  </section>
}
