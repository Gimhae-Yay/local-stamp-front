...
function ReservationCrumbs({ eventTitle, current }: { eventTitle: string; current: string }) {
  return <Breadcrumbs items={[{ label: '홈', to: '/' }, { label: '내 예약', to: '/reservations' }, { label: eventTitle }, { label: current }]} />
}
...
export function BookingPage() {
  ...
  const [people, setPeople] = useState({ adult: 2, child: 1, senior: 0 })
  const total = people.adult + people.child + people.senior
  const sessions: Array<[string, string, string, number]> = [
    ['17-10', '8월 17일 (일)', '10:00–12:00', 4], ['17-14', '8월 17일 (일)', '14:00–16:00', 8],
    ['23-10', '8월 23일 (토)', '10:00–12:00', 12], ['23-14', '8월 23일 (토)', '14:00–16:00', 0],
    ['30-10', '8월 30일 (토)', '10:00–12:00', 6], ['30-14', '8월 30일 (토)', '14:00–16:00', 10],
  ]
  const adjust = (key: keyof typeof people, amount: number) => setPeople((current) => ({ ...current, [key]: Math.max(0, Math.min(4, current[key] + amount)) }))
  return (
    <section className="page-container booking-page">
      ...
      <section className="booking-section"><div className="section-title"><h2>예약할 회차 선택</h2><span>총 6회차</span></div>
        <div className="session-list">{sessions.map(([id, date, time, seats]) => <button key={id} disabled={!seats} className={`session-row${selected === id ? ' selected' : ''}`} onClick={() => setSelected(id)}>
          <span className="date-box">{String(date).slice(3, 5)}</span><span><b>{time}</b><small>{event.address} · 2시간</small></span><StatusPill tone={!seats ? 'gray' : seats <= 6 ? 'amber' : 'green'}>{seats ? `${seats}자리 남음` : '정원 마감'}</StatusPill>
        </button>)}</div>
      </section>
      <section className="booking-section"><div className="section-title"><h2>예약 인원</h2><span>남은 자리 4명</span></div>
        {([['adult', '성인', '만 19세 이상'], ['child', '어린이', '만 6세 이상 18세 이하'], ['senior', '어르신', '만 65세 이상']] as const).map(([key, label, desc]) => <div className="counter-row" key={key}>
          <div><b>{label}</b><small>{desc}</small></div><div className="counter"><button onClick={() => adjust(key, -1)} aria-label={`${label} 감소`}>−</button><strong>{people[key]}</strong><button onClick={() => adjust(key, 1)} aria-label={`${label} 증가`}>＋</button></div>
        </div>)}
        <div className="total-people"><span>총 예약 인원</span><b>{total}명</b><span>최대 4명</span></div>
      </section>
      <button disabled={!total} className="button-primary booking-submit" onClick={() => navigate(`/events/${event.id}/reserve/confirm`)}>예약하기</button>
    </section>
  )
}
...
export function BookingConfirmPage() {
  ...
  return <section className="page-container narrow-page">
    ...
    <div className="confirmation-grid"><section><h2>예약 내용</h2><h3>{event.title}</h3><InfoRow label="선택 회차">8월 17일(일) 10:00–12:00</InfoRow><InfoRow label="위치">{event.location}</InfoRow></section><section><h2>예약 인원</h2><div className="people-summary"><span>총 예약 인원</span><b>3명</b></div><p className="summary-caption">성인 1명 · 어린이 1명 · 어르신 1명</p><InfoRow label="예약 금액">0원</InfoRow><button className="button-primary" onClick={() => navigate(`/events/${event.id}/reserve/complete`)}>예약 확정하기</button><p className="summary-caption">현장 혹은 내 예약에서 예약 QR을 확인할 수 있습니다.</p></section></div>
  </section>
}
...
export function ReservationsPage() {
  ...
  const upcomingReservations: Reservation[] = [
    { id: bookingId, date: '오늘 14:00–16:00', dateLabel: '2026. 08. 13 (목)', title: '김해 가야문화 체험', location: '김해문화회관 1층 · 김해시 가야의길 190', status: '체크인 가능', people: '총 3명 · 성인 1 · 어린이 1 · 어르신 1', checkin: true },
    { id: 'r20260824', date: '8월 23일(일) 13:00', dateLabel: '2026. 08. 23 (일)', title: '분청도자기 원데이 클래스', location: '김해시 진례면 · 김해문화센터', status: '예약 확정', people: '총 2명 · 성인 2', checkin: false },
  ]
  const pastReservations: Reservation[] = [
    { id: 'r20260730', date: '7월 30일(목) 14:00', dateLabel: '2026. 07. 30 (목)', title: '김해 가야문화 체험', location: '김해문화회관 1층 · 김해시 가야의길 190', status: '체크인 완료', people: '총 3명 · 성인 1 · 어린이 1 · 어르신 1', checkin: false, reviewable: true },
    { id: 'r20260718', date: '7월 18일(토) 10:00', dateLabel: '2026. 07. 18 (토)', title: '대성동 고분 박물관 해설', location: '대성동고분박물관 · 김해시 가야안길 126', status: '체크인 완료', people: '총 2명 · 성인 2', checkin: false, reviewable: true },
    { id: 'r20260629', date: '6월 29일(일) 11:00', dateLabel: '2026. 06. 29 (일)', title: '낙동강 생태 탐방', location: '김해시 생태원 일대', status: '체크인 완료', people: '총 1명 · 성인 1', checkin: false, reviewable: false },
  ]
...
export function PaymentCompletePage() {
  return <section className="payment-result"><Breadcrumbs items={[{ label: '홈', to: '/' }, { label: '김해 가야문화 체험', to: '/events/gaya-culture' }, { label: '예약' }, { label: '결제 결과' }]} />
