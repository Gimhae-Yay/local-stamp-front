...
function operatorSession(sessionId: string, day: string, overrides: Record<string, unknown> = {}) {
  return {
    sessionId,
    status: "SCHEDULED",
    version: 1,
...
describe("operator content integration", () => {
  it("creates a revision, navigates to it, and withdraws it", async () => {
    ...
    expect(screen.queryByRole("button", { name: "수정본 철회" })).not.toBeInTheDocument();
    expect(
  ...
  it("updates a cached rejected revision and reflects the result", async () => {
    ...
    expect(await screen.findByText("수정 결과가 저장되었습니다.")).toBeInTheDocument();
    expect(title).toHaveValue("수정 제목");
    expect(fetchMock.mock.calls[0]?.[0]).toBe("/api/v1/operator/content-revisions/52");
  ...
  it("creates, changes, and cancels sessions while reflecting each status", async () => {
    ...
    render(
      <MemoryRouter initialEntries={["/operator/contents/10"]}>
        <Routes>
          <Route path="/operator/contents/:contentId" element={<OperatorContentDetailPage />} />
        </Routes>
      </MemoryRouter>,
    );

    await user.click(await screen.findByRole("button", { name: "회차 생성" }));
    const createForm = screen.getByRole("button", { name: "회차 생성 요청" }).closest("form")!;
    await user.type(within(createForm).getByLabelText("시작 시각"), "2027-10-01T10:00");
    await user.type(within(createForm).getByLabelText("종료 시각"), "2027-10-01T12:00");
    await user.type(within(createForm).getByLabelText("체크인 시작"), "2027-10-01T09:30");
    await user.type(within(createForm).getByLabelText("체크인 종료"), "2027-10-01T11:30");
    await user.type(within(createForm).getByLabelText("정원"), "20");
    await user.click(within(createForm).getByRole("button", { name: "회차 생성 요청" }));
    expect(await screen.findByText("회차 #23")).toBeInTheDocument();

    const session21 = screen.getByText("회차 #21").closest("article")!;
    await user.click(within(session21).getByRole("button", { name: "변경 요청" }));
    await user.click(within(session21).getByRole("button", { name: "변경 심사 요청" }));
    expect(await within(session21).findByText("변경 심사 대기")).toBeInTheDocument();

    const session22 = screen.getByText("회차 #22").closest("article")!;
    await user.click(within(session22).getByRole("button", { name: "회차 취소" }));
    await user.type(within(session22).getByLabelText("취소 사유"), "기상 악화");
    await user.click(within(session22).getByRole("button", { name: "취소 확정" }));
    expect(await within(session22).findByText("취소됨")).toBeInTheDocument();
    expect(within(session22).queryByRole("button", { name: "회차 취소" })).not.toBeInTheDocument();
  });
  ...
  it("requests full content withdrawal and loads reservations for each session", async () => {
    ...
    await user.click(await screen.findByRole("button", { name: "전체 콘텐츠 철회 요청" }));
    await user.click(screen.getByRole("button", { name: "철회 요청 확정" }));
    expect(screen.getByText("전체 철회 사유를 입력해 주세요.")).toBeInTheDocument();
    await user.type(screen.getByLabelText("전체 철회 사유"), "운영 계획 변경");
    await user.click(screen.getByRole("button", { name: "철회 요청 확정" }));

    expect(await screen.findByText(/전체 콘텐츠 철회 요청이 접수되었습니다/)).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "전체 콘텐츠 철회 요청" })).not.toBeInTheDocument();
    ...
    expect(new Headers(withdrawalCall?.[1]?.headers).get("Idempotency-Key")).toMatch(/.+/);

    const session21 = screen.getByText("회차 #21").closest("article")!;
    await user.click(within(session21).getByRole("button", { name: "예약자 보기" }));
    ...
    const session22 = screen.getByText("회차 #22").closest("article")!;
    await user.click(within(session22).getByRole("button", { name: "예약자 보기" }));
    expect(await screen.findByText("이 회차에는 예약자가 없습니다.")).toBeInTheDocument();
  });
  ...
  it("shows server validation and duplicate command errors", async () => {
    const fetchMock = vi.fn().mockImplementation((input: string, init?: RequestInit) => {
      ...
      if (input === "/api/v1/operator/sessions/21/change-requests") {
        return Promise.resolve(
          errorResponse("SESSION_STATE_CONFLICT", "회차 상태가 요청을 처리할 수 없습니다.", 409),
        );
      }
      if (input === "/api/v1/operator/sessions/21/cancel") {
        return Promise.resolve(
          errorResponse("SESSION_NOT_CANCELLABLE", "취소할 수 없는 회차 상태입니다.", 409),
        );
      }
      return Promise.reject(new Error(`unexpected request: ${input}`));
    });
    ...
    await user.click(await screen.findByRole("button", { name: "회차 생성" }));
    const createForm = screen.getByRole("button", { name: "회차 생성 요청" }).closest("form")!;
    await user.type(within(createForm).getByLabelText("시작 시각"), "2027-10-01T12:00");
    await user.type(within(createForm).getByLabelText("종료 시각"), "2027-10-01T12:00");
    await user.type(within(createForm).getByLabelText("체크인 시작"), "2027-10-01T09:30");
    await user.type(within(createForm).getByLabelText("체크인 종료"), "2027-10-01T11:30");
    await user.type(within(createForm).getByLabelText("정원"), "20");
    await user.click(within(createForm).getByRole("button", { name: "회차 생성 요청" }));
    expect(
      await within(createForm).findByText("종료 시각은 시작 시각보다 뒤여야 합니다."),
    ).toBeInTheDocument();
    ...
    await user.click(within(session).getByRole("button", { name: "변경 심사 요청" }));
    expect(
    ...
    expect(
      await within(session).findByText("이미 취소됐거나 현재 취소할 수 없는 회차입니다."),
    ).toBeInTheDocument();
  });
});
