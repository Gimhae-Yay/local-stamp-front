...
export default function OperatorRequestPage() {
  ...
  return (
    <section className="page-container narrow-page">
      <Breadcrumbs
        items={[{ label: "홈", to: "/" }, { label: "운영자 재신청" }]}
      />
      <PageHeader
        title="운영자 재신청"
      ...
      {result ? (
        <section className="operator-request-result">
          ...
          <p>
            {requestedRegion?.name ?? `지역 #${result.requestedRegionId}`} 담당
            관리자가 신청 내용을 검토합니다.
          </p>
        ...
        <>
          <Notice>
            이 화면은 이전 신청이 반려된 활성 회원만 사용할 수 있습니다. 최초
            운영자 신청, 심사 중인 신청, 이미 운영자 권한이 있는 계정은 Backend
            정책에 따라 제출되지 않습니다.
          </Notice>
          <form className="operator-request-form" onSubmit={submit}>
            ...
            <label>
              ...
              <span className="character-count">
                {businessInformation.length} / 2,000자
              </span>
            </label>
            ...
            <button
              ...
              type="submit"
              disabled={
                submitting || !requestedRegionId || !businessInformation.trim()
              }
            >
