...
describe("operator reservation search integration", () => {
  it("validates an empty number and displays success, not found, and forbidden results", async () => {
    const fetchMock = vi.fn().mockImplementation((input: string) => {
      ...
      if (
        input === "/api/v1/operator/reservations/search?reservationNo=MISSING"
      ) {
        return Promise.resolve(
          errorResponse("NOT_FOUND", "예약을 찾을 수 없습니다.", 404),
        )
      }
      if (
        input === "/api/v1/operator/reservations/search?reservationNo=FOREIGN"
      ) {
        return Promise.resolve(
    ...
    await user.click(screen.getByRole("button", { name: "검색" }))
    expect(
      await screen.findByText(/일치하는 예약번호를 찾을 수 없습니다/),
    ).toBeInTheDocument()
