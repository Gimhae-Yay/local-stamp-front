...
const ongoingExps: Experience[] = [
  ...
  { id: 2, title: '대성동 고분 박물관 해설', location: '김해시 가야의길 126', time: '오늘 11:00–17:00',      badge: '진행 중',  seats: '다음 회차 8자리 남음' },
...
export default function App() {
  ...
  const applyFilter = (list: Experience[]) =>
    filter === '예약 가능' ? list.filter(e => e.badge === '예약 가능') : list

  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      <Navbar loggedIn={loggedIn} onToggleLogin={() => setLoggedIn(v => !v)} />
      ...
      <ExperienceSection
        title="지금 진행 중인 체험"
        items={applyFilter(ongoingExps)}
        right={`진행 중인 콘텐츠 ${applyFilter(ongoingExps).length}개`}
      />

      <ExperienceSection
        title="곧 시작하는 체험"
        items={applyFilter(upcomingExps)}
        right={<span style={{ cursor: 'pointer' }}>전체 행사·체험 보기 ›</span>}
      />

      {loggedIn ? <ActivitySection /> : <LoginCTA onLogin={() => setLoggedIn(true)} />}
    </div>
  )
}
