...
export function createPayment(holdId: string, couponId: string | null, idempotencyKey: string) {
...
export function getMyReservations(signal?: AbortSignal) {
  return apiRequest<{ reservations: ReservationSummary[] }>("/api/v1/me/reservations", { signal });
}
...
export function getMyPayment(paymentId: string, signal?: AbortSignal) {
  return apiRequest<PaymentDetail>(`/api/v1/me/payments/${encodeURIComponent(paymentId)}`, {
    signal,
  });
}
...
export function getMyRefund(refundId: string, signal?: AbortSignal) {
  return apiRequest<Refund>(`/api/v1/me/refunds/${encodeURIComponent(refundId)}`, { signal });
}
