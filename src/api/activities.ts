...
export interface MissionParticipationDetail
  extends MissionParticipationSummary {
  conditionType: string
  progresses: Array<{
    visitId: string
    contentId: string
...
export function getPublicMission(missionId: string, signal?: AbortSignal) {
  return apiRequest<PublicMissionDetail>(
...
export function getMyMissionParticipation(
  participationId: string,
  signal?: AbortSignal,
) {
...
export function getMyStampbooks(signal?: AbortSignal) {
  return apiRequest<{ stampbooks: StampbookSummary[] }>(
    "/api/v1/me/stampbooks",
    { signal },
  )
}

export function getMyStampbook(stampbookId: string, signal?: AbortSignal) {
  return apiRequest<StampbookDetail>(
    `/api/v1/me/stampbooks/${encodeURIComponent(stampbookId)}`,
    { signal },
  )
}

export function getMyStampbookEarnings(
  stampbookId: string,
  signal?: AbortSignal,
) {
...
export function getMyCoupons(status?: string, signal?: AbortSignal) {
  return apiRequest<{ coupons: CouponSummary[] }>(
    withQuery("/api/v1/me/coupons", { status }),
    { signal },
  )
}
...
export function getMyCoupon(couponId: string, signal?: AbortSignal) {
  return apiRequest<CouponDetail>(
    `/api/v1/me/coupons/${encodeURIComponent(couponId)}`,
    { signal },
  )
}

export function getMyCouponUsageHistory(
  couponId: string,
  signal?: AbortSignal,
) {
...
export function createVisitReview(
  ...
  return apiRequest<ReviewMutationResponse & {
    visitId: string
    contentId: string
