...
describe("운영자 결제 조회·체크인 프론트 흐름", () => {
  ...
  it("현재 회차가 비어 있어도 예약번호로 과거 환불 상태를 조회한다", async () => {
    ...
    const fetchMock = vi.fn((input: RequestInfo | URL) => {
      ...
      if (url.endsWith("/api/v1/operator/contents/104/reservations?sessionId=204")) {
        return success({
          contentId: "104",
      ...
      if (url.endsWith("/api/v1/operator/reservations/search?reservationNo=R-REFUND-100")) {
  ...
  it("수동 체크인은 계약된 사유를 보내고 연속 클릭을 한 요청으로 제한한다", async () => {
    window.history.replaceState({}, "", "/operator/check-in?reservationNo=R-CHECK-IN-101");
    ...
    const fetchMock = vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
      ...
      const auth = authResponse(url);
      if (auth) return auth;
      if (url.endsWith("/api/v1/operator/reservations/search?reservationNo=R-CHECK-IN-101")) {
        return success(reservation);
      }
      if (url.endsWith("/api/v1/operator/check-ins/manual") && init?.method === "POST") {
    ...
    const manualRequests = fetchMock.mock.calls.filter(
      ([input, init]) =>
        String(input).endsWith("/api/v1/operator/check-ins/manual") && init?.method === "POST",
    );
    ...
    expect(new Headers(manualRequests[0]?.[1]?.headers).get("Idempotency-Key")).toBeTruthy();
    ...
    expect(screen.getAllByText("김*자 · 010-****-5678").length).toBeGreaterThan(0);
    expect(screen.getByRole("button", { name: "예약번호로 체크인" })).toBeDisabled();
  });
  ...
  it("QR 성공 후 예약번호를 보강 조회해 예약자 정보를 표시한다", async () => {
    ...
    const fetchMock = vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
      ...
      const auth = authResponse(url);
      if (auth) return auth;
      if (url.endsWith("/api/v1/operator/check-ins") && init?.method === "POST") {
        return success({
          visitId: "301",
          reservationId: "101",
          sessionId: "204",
    ...
    render(<App />);
    fireEvent.change(await screen.findByPlaceholderText("스캐너가 전달한 QR 토큰"), {
