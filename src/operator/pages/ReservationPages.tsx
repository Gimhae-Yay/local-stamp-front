...
export function ReservationPage() {
  ...
  useEffect(() => {
    ...
    listMyContents(controller.signal)
      .then(({ contents: items }) => {
        ...
        if (!selectedContent && published[0])
          setSelectedContent(published[0].contentId)
      })
      .catch((caught) => {
  ...
  useEffect(() => {
    ...
    setPayment(null)
    setSearchParams(
      { contentId: selectedContent, sessionId: selectedSession },
      { replace: true },
    )
    listSessionReservations(selectedContent, selectedSession, controller.signal)
      ...
      .catch((caught) => {
        if (!isAbortError(caught, controller.signal))
          setError(
            apiErrorMessage(caught, "예약자 목록을 불러오지 못했습니다."),
          )
      })
      .finally(() => {
        if (!controller.signal.aborted) setLoading(false)
  ...
  return (
    <>
      ...
      <div className="op-tabs">
        <button
          className={`op-tab${tab === "qr" ? " active" : ""}`}
          onClick={() => setTab("qr")}
        >
          QR 스캔
        </button>
      ...
      {tab === "qr" ? (
        ...
        <ManualCheckIn
          initialReservationNo={searchParams.get("reservationNo") ?? ""}
        />
      )}
    </>
  )
