...
describe("쿠폰 정책 입력 검증", () => {
  ...
  it("금액 관계, 유효일 상한, 발급 기간 순서를 검사한다", () => {
    expect(validateCouponDraft(validDraft({ minimumPaymentAmount: "2999" }))).toContain(
      "할인 금액 이상",
    );
    expect(validateCouponDraft(validDraft({ validDaysAfterIssue: "366" }))).toContain("365일 이하");
    expect(validateCouponDraft(validDraft({ issueEndsAt: "2026-08-01T09:00" }))).toContain(
      "시작 시각보다 뒤",
    );
  });

  it("정수 형식과 수정 사유를 검사한다", () => {
    expect(validateCouponDraft(validDraft({ discountAmount: "1.5" }))).toContain("정수");
    expect(validateCouponDraft(validDraft({ reason: "" }), true)).toContain("수정 사유");
  });
});
...
describe("쿠폰 정책 화면 상태 처리", () => {
  ...
  it("생성 폼은 설명을 선택값으로 두고 입력 길이와 유효일을 제한한다", async () => {
    render(
      <MemoryRouter initialEntries={["/operator/coupon-policies/new"]}>
        <Routes>
          <Route path="/operator/coupon-policies/new" element={<CouponFormPage />} />
        </Routes>
      </MemoryRouter>,
    );
    ...
    expect(screen.getByLabelText("정책 설명")).toHaveAttribute("maxlength", "1000");
    expect(screen.getByLabelText("발급 후 유효일")).toHaveAttribute("max", "365");
  });
  ...
  it("종료된 정책의 전체 상세와 상태를 표시하고 변경 버튼을 숨긴다", async () => {
    ...
    expect(screen.queryByRole("button", { name: "정책 공개" })).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "정책 종료" })).not.toBeInTheDocument();
  });
});
...
describe("미션·스탬프북 종료 흐름", () => {
  ...
  it("심사 대기 필터에 서버 계약 상태값을 전송한다", async () => {
    ...
    await waitFor(() =>
      expect(listMissions).toHaveBeenCalledWith("PENDING_REVIEW", 0, 20, expect.any(AbortSignal)),
    );
  });
  ...
  it("방문 횟수 미션은 대상 콘텐츠 없이 생성할 수 있다", async () => {
    ...
    render(
      <MemoryRouter initialEntries={["/operator/missions/new"]}>
        <Routes>
          <Route path="/operator/missions/new" element={<MissionFormPage />} />
          <Route path="/operator/missions" element={<div>목록 이동 완료</div>} />
        </Routes>
      </MemoryRouter>,
    );
  ...
  it("미션 종료 사유를 Backend 허용 코드 중에서 선택한다", async () => {
    ...
    expect(await screen.findByText("김해 골목 세 곳 방문하기")).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "미션 종료" }));
    ...
    await waitFor(() =>
      expect(endMission).toHaveBeenCalledWith("701", "MISSION_OPERATION_SCHEDULE_CHANGED"),
    );
  });
  ...
  it("서버 상세 URL에서 종료 사유 확인 후 스탬프북 종료 API를 호출한다", async () => {
    ...
    render(
      <MemoryRouter initialEntries={["/operator/stampbooks/801"]}>
        <Routes>
          <Route path="/operator/stampbooks/:stampbookId" element={<StampbookDetailPage />} />
        </Routes>
      </MemoryRouter>,
    );

    fireEvent.click(await screen.findByRole("button", { name: "스탬프북 종료" }));
    const dialog = screen.getByRole("dialog");
    fireEvent.change(within(dialog).getByRole("textbox"), {
      target: { value: "행사 운영 종료" },
    });
    fireEvent.click(within(dialog).getByRole("button", { name: "스탬프북 종료" }));

    await waitFor(() => expect(endStampbook).toHaveBeenCalledWith("801", "행사 운영 종료"));
  ...
  it("직접 수정 URL에서 Backend 상세를 불러와 PATCH 요청한다", async () => {
    ...
    render(
      <MemoryRouter initialEntries={["/operator/stampbooks/801/edit"]}>
        <Routes>
          <Route path="/operator/stampbooks/:stampbookId/edit" element={<StampbookFormPage />} />
          <Route path="/operator/stampbooks/:stampbookId" element={<div>상세 이동 완료</div>} />
        </Routes>
      </MemoryRouter>,
    );
