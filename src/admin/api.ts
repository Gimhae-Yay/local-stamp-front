...
export async function getRegionAdminAssignment(): Promise<RoleAssignment> {
  const result = await apiRequest<{ roleAssignments: RoleAssignment[] }>(
    "/api/v1/me",
  )
  const assignment = result.roleAssignments.find(
    (item) => item.role === "REGION_ADMIN",
  )
  if (!assignment?.regionId)
    throw new ApiError(
      "활성 지역 관리자 권한을 확인할 수 없습니다.",
      403,
