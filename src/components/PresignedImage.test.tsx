...
function RefreshProbe({ expiresAt, onRefresh }: { expiresAt: string; onRefresh: () => void }) {
...
describe("PresignedImage", () => {
  ...
  it("refreshes ten seconds before the earliest valid expiration", () => {
    ...
    expect(
      getPresignedImageRefreshDelay(["2026-08-19T00:05:00Z", "2026-08-19T00:03:00Z", null], now),
    ).toBe(170_000);
  });
  ...
  it("invokes the refresh callback shortly before expiration", () => {
    ...
    const onRefresh = vi.fn();
    render(<RefreshProbe expiresAt="2026-08-19T00:03:00Z" onRefresh={onRefresh} />);
