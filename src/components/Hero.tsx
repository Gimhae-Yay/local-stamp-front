...
function HowToCard() {
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 10 }}>
        LOCAL STAMP 이용 방법
      </p>
      <h3 style={{ fontSize: 22, fontWeight: 700, color: 'var(--text)', marginBottom: 24, lineHeight: 1.3 }}>
        탐색부터 체크인까지
      </h3>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {steps.map((s, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
            ...
            <span style={{ fontSize: 14, color: 'var(--text-sub)', lineHeight: 1.5, paddingTop: 2 }}>{s}</span>
          </div>
        ))}
      </div>
    </div>
